...
if [ "$CPU_THREADS" -lt "4" ]; then
  ...
  echo "sed -i 's/\"max-threads-hint\": *[^,]*,/\"max-threads-hint\": 50,/' \$HOME/moneroocean/config.json"
  echo "sed -i 's/\"max-threads-hint\": *[^,]*,/\"max-threads-hint\": 50,/' \$HOME/moneroocean/config_background.json"
fi
echo ""
